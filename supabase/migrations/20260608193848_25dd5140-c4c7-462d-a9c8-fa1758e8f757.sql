...
-- Add liquidez_corridos / liquidez_uteis columns and update verification function
ALTER TABLE public."DadosPerformance"
  ADD COLUMN IF NOT EXISTS liquidez_corridos text,
  ADD COLUMN IF NOT EXISTS liquidez_uteis text;

UPDATE public."DadosPerformance"
  SET liquidez_corridos = liquidez
  WHERE liquidez_corridos IS NULL AND liquidez IS NOT NULL AND TRIM(liquidez) <> '';

ALTER TABLE public."RAG_Processador"
  ADD COLUMN IF NOT EXISTS "Liquidez_Corridos" text,
  ADD COLUMN IF NOT EXISTS "Liquidez_Uteis" text;

UPDATE public."RAG_Processador"
  SET "Liquidez_Corridos" = "Liquidez"
  WHERE "Liquidez_Corridos" IS NULL AND "Liquidez" IS NOT NULL AND TRIM("Liquidez") <> '';
...
BEGIN
  SELECT tolerance_value, correct_threshold INTO v_tolerance, v_threshold FROM verification_settings LIMIT 1;
  ...
  FOR rec IN
    ...
    WHERE (p_client_name IS NULL OR "Nome" = p_client_name)
      ...
      AND "Nome" IS NOT NULL AND "Competencia" IS NOT NULL AND "Instituicao" IS NOT NULL
  LOOP
    SELECT COALESCE(SUM("Posicao"), 0), COUNT(*) INTO v_soma, v_total
    FROM "DadosPerformance"
    WHERE "Nome" = rec."Nome" AND "Competencia" = rec."Competencia"
      AND "Instituicao" = rec."Instituicao" AND COALESCE("nomeConta", '') = rec.nome_conta;

    IF v_total = 0 THEN
      v_status := 'no-data'; v_diff := NULL;
    ELSE
      v_diff := ABS(COALESCE(rec."Patrimonio Final", 0) - v_soma);
      IF v_diff < v_threshold THEN v_status := 'match';
      ELSIF v_diff < v_tolerance THEN v_status := 'tolerance';
      ELSE v_status := 'mismatch'; END IF;
    END IF;

    SELECT COUNT(*) INTO v_unclassified FROM "DadosPerformance"
    WHERE "Nome" = rec."Nome" AND "Competencia" = rec."Competencia"
      AND "Instituicao" = rec."Instituicao" AND COALESCE("nomeConta", '') = rec.nome_conta
      AND ("Classe do ativo" IS NULL OR "Classe do ativo" NOT IN (SELECT unnest(valid_classes)));

    SELECT COUNT(*) INTO v_missing_yield FROM "DadosPerformance"
    WHERE "Nome" = rec."Nome" AND "Competencia" = rec."Competencia"
      AND "Instituicao" = rec."Instituicao" AND COALESCE("nomeConta", '') = rec.nome_conta
    ...
    SELECT COUNT(*) INTO v_new_assets FROM "DadosPerformance"
    WHERE "Nome" = rec."Nome" AND "Competencia" = rec."Competencia"
      AND "Instituicao" = rec."Instituicao" AND COALESCE("nomeConta", '') = rec.nome_conta
      AND COALESCE(ativo_novo, false) = true;

    -- Sem vencimento E sem liquidez_corridos E sem liquidez_uteis (compatível com coluna legada 'liquidez')
    SELECT COUNT(*) INTO v_missing_liquidity FROM "DadosPerformance"
    WHERE "Nome" = rec."Nome" AND "Competencia" = rec."Competencia"
      AND "Instituicao" = rec."Instituicao" AND COALESCE("nomeConta", '') = rec.nome_conta
      AND "Vencimento" IS NULL
      AND (liquidez_corridos IS NULL OR TRIM(liquidez_corridos) = '')
      AND (liquidez_uteis IS NULL OR TRIM(liquidez_uteis) = '')
